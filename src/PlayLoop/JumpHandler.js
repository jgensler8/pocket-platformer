...
export class JumpHandler {
    ...
    static performJump(jumpSpeed, maxFrames) {
        ...
        if (player.forcedJumpSpeed !== 0 && player.jumpframes >= maxFrames) {
    ...
    static resetWallJump(wallJumpDirection) {
        ...
        player.currentWallJumpCoyoteFrame = 0;
        ...
        player.wallJumpDirection = wallJumpDirection;
    }
    ...
    static fullySubmergedInWater() {
        const player = window.player;
        return player.top_right_pos_in_water && player.top_left_pos_in_water
            && player.bottom_right_pos_in_water && player.bottom_left_pos_in_water;
    }
    ...
    static swimHandler() {
        ...
        const waterAtTopOnly = !player.top_right_pos_in_water && !player.top_left_pos_in_water
            && (player.bottom_right_pos_in_water || player.bottom_left_pos_in_water);
        ...
        if (player.yspeed < 0 && waterAtTopOnly && player.forcedJumpSpeed === 0) {
        ...
        else if (!player.flapped && Controller.jumpReleased) {
            ...
            if (player.yspeed - player.flapHeight > player.maxSwimHeight) {
                player.yspeed -= player.flapHeight;
            }
            else {
                player.yspeed = player.maxSwimHeight;
            }
